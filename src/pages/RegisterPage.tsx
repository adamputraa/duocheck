/**
 * Register page for DuoCare.
 * Sign-up form with display name, email, password, role selection.
...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!role) { setRoleError('Please select your role.'); return }
    setRoleError(null)
    clearError()
    try {
  ...
  const handleOtpChange = (index: number, value: string) => {
    ...
    if (digit && index === 5 && newCode.every(c => c !== '')) handleVerifyOtp(newCode.join(''))
  }

  const handleOtpKeyDown = (index: number, e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'Backspace' && !otpCode[index] && index > 0) {
      otpRefs.current[index - 1]?.focus()
      const newCode = [...otpCode]; newCode[index - 1] = ''; setOtpCode(newCode)
    }
  }
  ...
  const handleOtpPaste = (e: React.ClipboardEvent) => {
    ...
    if (pastedData.length > 0) {
      ...
      else handleVerifyOtp(newCode.join(''))
    }
  }

  const handleVerifyOtp = async (code?: string) => {
    const token = code || otpCode.join('')
    if (token.length !== 6) { setVerifyError('Please enter all 6 digits.'); return }
    setVerifyError(null)
    try { await verifyOtp(email, token) }
    catch { setVerifyError('Invalid code. Please check and try again.') }
  }
      ...
      <div className="min-h-dvh bg-cream flex flex-col items-center justify-center p-6">
        <div className="w-full max-w-sm">
          <div className="text-center mb-8">
            ...
            <p className="text-text-muted mt-1">We sent a 6-digit code to</p>
            <p className="text-text-dark font-medium text-sm mt-0.5">{email}</p>
          </div>

          {(error || verifyError) && (
            <div className="bg-emergency/10 border border-emergency/20 text-emergency rounded-xl p-4 mb-4 text-sm">{verifyError || error}</div>
          )}
          ...
          <div className="mt-6 text-center">
            ...
            <button onClick={async () => {
              clearError(); setVerifyError(null); setOtpCode(['','','','','',''])
              try { if (role) await signUp(email, password, displayName, role) } catch {}
            }} disabled={loading} className="text-primary font-semibold text-sm hover:text-primary-dark transition disabled:opacity-50 mt-1">
              Resend code
            </button>
          </div>

          <p className="text-center text-sm text-text-muted mt-4">
            <button onClick={() => { setOtpSent(false); setOtpCode(['','','','','','']); setVerifyError(null); clearError() }}
    ...
    <div className="min-h-dvh bg-cream flex flex-col items-center justify-center p-6">
      <div className="w-full max-w-sm">
        <div className="text-center mb-8">
          ...
          <p className="text-text-muted mt-1">Join DuoCare — your pregnancy companion</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label htmlFor="displayName" className="block text-sm font-medium text-text-dark mb-1.5">Display Name</label>
          ...
          {/* Role selection */}
          <div>
            <label className="block text-sm font-medium text-text-dark mb-1.5">I am the...</label>
            <div className="flex gap-2">
              {(['wife', 'husband'] as const).map(r => (
                <button key={r} type="button" onClick={() => { setRole(r); setRoleError(null) }}
                  className={`flex-1 px-4 py-3 rounded-xl border text-sm font-semibold transition-colors min-h-[44px] ${
                    role === r ? 'bg-primary text-white border-primary' : 'bg-white text-text-dark border-border-light active:bg-primary-light/50'
                  }`}>
                  {r === 'wife' ? '👩 Wife' : '👨 Husband'}
                </button>
              ))}
            </div>
            {roleError && <p className="text-xs text-emergency mt-1">{roleError}</p>}
          </div>
        ...
        <div className="flex items-start gap-2 mt-4 p-3 bg-primary-light/30 rounded-xl">
          ...
          <p className="text-xs text-text-muted">Your data is private. Only your partner can see your check-ins.</p>
        </div>

        <p className="text-center text-sm text-text-muted mt-6">
          Already have an account?{' '}
          <Link to="/login" className="text-primary font-semibold hover:text-primary-dark transition">Sign in</Link>
        </p>
      </div>
    </div>
  )
